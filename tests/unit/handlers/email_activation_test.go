...
func Test_GenerateActionToken(t *testing.T) {
	...
	fakeUsers := factory.Generate(&models.User{}, map[string]interface{}{}, 1)
	...
	tokenString, err := handlers_v1.GenerateActivationToken(user.Email, newSession)

	assert.Nil(t, err)
	assert.NotNil(t, tokenString)
	assertion.DatabaseHas(t, &models.EmailVerify{}, map[string]string{"email": user.Email}, db)
}
